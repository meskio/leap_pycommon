...
class _HTTP11ClientProtocol(HTTP11ClientProtocol):
    ...
    def request(self, request):
        ...
        if self._timeout:
            ...
            self._finishedRequest.addBoth(self._cancelTimeout, timeoutCall)
        return d
    ...
    def _doTimeout(self, request):
        ...
        :type request: twisted.web._newclient.Request
        """
        buffer_len = len(self._parser._buffer)
    ...
    def _cancelTimeout(self, passthrough, timeoutCall):
        """
        Cancel the request timeout, when it's finished.

        :param passthrough: The result of previous callback.
